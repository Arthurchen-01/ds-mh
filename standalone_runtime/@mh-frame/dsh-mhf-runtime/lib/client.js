...
function responseSummary(text) {
	...
	return normalized === "" ? "空响应" : normalized.slice(0, 160);
}
...
async function readJsonResponse(response, serviceName) {
	...
	try {
		...
		if (serviceName === "local" && response.status === 404) {
			throw new Error(`MH Frame 插件运行时未加载（HTTP 404）：${summary}`);
		}
		throw new Error(`MH Frame ${serviceName === "local" ? "本地服务" : "服务"}响应格式无效（HTTP ${response.status}）：${summary}`);
	}
}
...
class MhfSettingsScope {
	...
	async request(method, payload) {
		...
		const body = await readJsonResponse(response, "local");
	...
	async refresh() {
		try {
			...
			this.publish({ ...this.state, status: "unavailable", writable: false, error: errorMessage(error) });
		}
	}
	enqueue(payload) {
		const write = async () => {
...
function CustomPromptEditor({ scope, value, disabled, t }) {
	const [draft, setDraft] = useState(null);
	const [saving, setSaving] = useState(false);
	const [saved, setSaved] = useState(false);
	const text = draft ?? value.customSystemPrompt ?? "";
	const enabled = value.customSystemPromptEnabled ?? false;
	const dirty = text !== (value.customSystemPrompt ?? "");
	const setEnabled = async (checked) => {
		setSaving(true);
		setSaved(false);
		try {
			const ok = checked
				? await scope.enqueue({ patch: { customSystemPrompt: text, customSystemPromptEnabled: true } })
				: await scope.set("customSystemPromptEnabled", false);
			if (ok && checked) {
				setDraft(null);
				setSaved(true);
			}
	...
	const save = async () => {
		...
		setSaved(false);
		try {
			if (await scope.set("customSystemPrompt", text)) {
				setDraft(null);
				setSaved(true);
			}
		} finally {
	...
	return jsxs("div", {
		className: "mhf-section-block mhf-custom-prompt",
		children: [
			jsxs("div", {
				className: "mhf-control-row",
				children: [
					jsxs("div", { children: [
						jsx("strong", { children: t("customPrompt") }),
						jsx("p", { children: t("customPromptDescription") })
					] }),
					Toggle({ checked: enabled, disabled: disabled || saving || (!enabled && !text.trim()), onChange: (checked) => void setEnabled(checked), label: t("customPrompt") })
				]
			}),
			jsx("label", { htmlFor: "mhf-custom-prompt-input", children: t("customPromptText") }),
			jsx("textarea", {
				id: "mhf-custom-prompt-input",
				className: "mhf-prompt-textarea",
				rows: 8,
				value: text,
			...
			jsxs("div", {
				className: "mhf-prompt-footer",
				children: [
					jsx("span", { role: "status", children: !text.trim() ? t("customPromptEmpty") : dirty ? t("customPromptUnsaved") : saved ? t("customPromptSaved") : t("customPromptHint") }),
					jsx("button", {
						type: "button",
						className: "mhf-button mhf-button-primary",
						disabled: disabled || saving || !dirty || (enabled && !text.trim()),
						"aria-busy": saving,
						onClick: () => void save(),
						children: saving ? t("customPromptSaving") : t("customPromptSave")
					})
				]
			})
		]
...
function FlashThinkingControl({ scope, value, disabled, t }) {
	const threshold = value.flashThinkingRestoreThreshold ?? 30;
	return jsxs("div", { className: "mhf-section-block mhf-flash-control", children: [
		jsx("label", { htmlFor: "mhf-flash-threshold", children: `${t("flashThreshold")}：${threshold}%` }),
		jsx("input", { id: "mhf-flash-threshold", type: "range", min: 5, max: 50, step: 5, value: threshold, disabled, onChange: (event) => scope.set("flashThinkingRestoreThreshold", Number(event.target.value)) }),
		jsxs("div", { className: "mhf-prompt-footer", children: [
			jsx("span", { children: t("flashThresholdWarning") }),
			jsx("button", { type: "button", className: "mhf-button", disabled: disabled || threshold === 30, onClick: () => scope.set("flashThinkingRestoreThreshold", 30), children: t("flashThresholdReset") })
		] })
	] });
}
...
function MhfSettingsSection({ scope, t }) {
	...
	if (state.status === "unavailable") {
		return jsxs("section", {
			className: "mhf-settings mhf-authorization-only mhf-runtime-unavailable",
			children: [
				jsx("img", { className: "mhf-auth-mark", src: MHF_ICON_URL, alt: "" }),
				jsxs("div", { className: "mhf-auth-copy", children: [
					jsx("h2", { children: t("runtimeUnavailableTitle") }),
					jsx("p", { children: state.error ?? t("runtimeUnavailableDescription") })
				] }),
				jsx("button", {
					type: "button",
					className: "mhf-button mhf-button-primary",
	...
	return jsxs("section", {
		...
		children: [
			...
			jsxs("div", {
				...
				children: [
					jsxs("div", { children: [
						...
						jsx("p", { children: enabled ? value?.customSystemPromptEnabled ? t("coreCustomEnabledDesc") : t("coreEnabledDesc") : t("coreDisabledDesc") })
					] }),
					...
					promptModes.length === 0
						...
						: jsx("div", {
							...
							children: promptModes.map((item) => jsx(OnlinePromptRow, {
								item,
								active: !value?.customSystemPromptEnabled && item.slug === activeMode,
								disabled: disabled || checkingMode !== null,
			...
			state.runtimeStatus?.developerAccount ? jsx(CustomPromptEditor, { scope, value, disabled, t }) : null,
			jsx(FlashThinkingControl, { scope, value, disabled, t }),
			jsxs("div", {
			...
			jsxs("button", {
				...
				children: [
					...
					wide && jsxs("span", { className: "mhf-sidebar-copy", children: [
						...
						jsx("small", { children: authorizationStatus === "protected" ? t("protectionTriggeredTitle") : enabled ? (authorizationStatus === "unauthorized" ? t("unauthorized") : authorizationStatus === "checking" ? t("checkingAuthorization") : state.value?.customSystemPromptEnabled ? t("customPrompt") : activePrompt ? `${activePrompt.name} ${activePrompt.version || "-"}` : t("noActivePackage")) : t("disabled") })
					] })
				]
			}),
			jsx("span", {
...
`;
	...
	style.textContent += `.mhf-custom-prompt{gap:10px}.mhf-custom-prompt>.mhf-control-row{padding-top:0}.mhf-custom-prompt>label{font-size:13px;font-weight:600}.mhf-prompt-textarea{display:block;width:100%;min-height:160px;resize:vertical;padding:12px;border:1px solid rgba(127,127,127,.3);border-radius:6px;background:var(--ds-bg-primary,#fff);color:var(--ds-text-primary,#111827);font:13px/1.6 Consolas,monospace;outline:none}.mhf-prompt-textarea:focus{border-color:#059669;box-shadow:0 0 0 3px rgba(16,185,129,.12)}.mhf-prompt-textarea:disabled{opacity:.6}.mhf-prompt-footer{display:flex;align-items:center;justify-content:space-between;gap:12px}.mhf-prompt-footer>span{font-size:12px;color:var(--ds-text-secondary,#6b7280)}.mhf-prompt-footer>.mhf-button{flex:none}`;
...
const zh = {
	...
	disabled: "已关闭",
	coreSwitch: "破甲开关",
	coreEnabledDesc: "当前使用在线模式提示词，重复注入两次。",
	coreCustomEnabledDesc: "当前仅使用自定义提示词，重复注入两次；在线提示词不参与注入。",
	...
	onlinePrompts: "在线提示词",
	onlinePromptsDescription: "提示词由服务器按当前设备授权下发。点击启用在线模式会关闭自定义提示词。",
	customPrompt: "自定义提示词",
	customPromptDescription: "开启后只使用自定义内容，重复注入两次。关闭后恢复所选在线模式，并保留文字。",
	customPromptText: "提示词内容",
	customPromptPlaceholder: "输入你希望补充的回答风格、格式要求或任务规则…",
	customPromptHint: "开启开关会保存并使用当前内容；后续编辑需点击保存，从下一次请求生效。",
	customPromptUnsaved: "有未保存的修改。开启自定义时会同时保存；已开启时请点击保存。",
	customPromptEmpty: "请输入自定义提示词后再开启。",
	customPromptSaved: "已保存",
	customPromptSave: "保存提示词",
	customPromptSaving: "保存中…",
	flashThreshold: "阈值设置",
	flashThresholdWarning: "稳定性，非必请勿调整",
	flashThresholdReset: "重置",
	...
	runtimeUnavailableTitle: "插件运行时未加载",
	runtimeUnavailableDescription: "MH Frame 本地服务不可用，请使用最新版插件管理器重新安装插件，然后重启 Harness。",
