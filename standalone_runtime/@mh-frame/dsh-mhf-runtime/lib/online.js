...
async function readOnlineJsonResponse(response) {
	const text = await response.text();
	try {
		return JSON.parse(text);
	} catch {
		throw new Error(`服务器响应格式无效（HTTP ${response.status}）：${responseSummary(text)}`);
	}
}
...
async function onlineJsonRequest(path, options = {}) {
	...
	const body = await readOnlineJsonResponse(response);
	if (!response.ok) {
...
export { decryptOnlinePrompt, deriveMacDeviceId, deriveWindowsDeviceId, fetchOnlinePrompts, int64Buffer, loginOnlineAccount, readDeviceId, readMacDeviceId, readOnlineJsonResponse, readWindowsDeviceId };
